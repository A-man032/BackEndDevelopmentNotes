...
| 类型  | 存储需求 |                      取值范围                      |
| :---: | :------: | :------------------------------------------------: |
|  int  |  4字节   | -2<sup>31 </sup>~ 2<sup>31</sup>-1（刚刚超过20亿） |
...
- 在子类中保存抽象类中的部分或所有抽象方法仍未定义，子类也被标为抽象类
- 定义全部方法，子类就不是抽象类了

**抽象类的对象变量** 只能引用非抽象子类的对象。

> **与C++的不同：** C++中一种抽象方法称为**纯虚函数** ，要在末尾用`=0` 标记。如果至少有一个纯虚函数，这个C++类就是抽象类。
...
- Object类型的变量可以引用任何类型的对象
  - Java中，基本类型不是对象。
  - 所有的数组类型（对象数组和基本类型的数组都扩展了Object类）
...
- ArrayList类类似于数组，但在添加或删除元素时，它能自动地调整数组容量，而不需要为此编写任何代码。
